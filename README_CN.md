...
如下 GIF 所示，DanmakuKit 提供三种类型的弹幕：悬浮、置顶和置地。
...
2. 实现一个继承自 DanmakuCell 的 View。接着需要重写 `displaying` 方法，并使用 CGContext 来绘制弹幕。**需要注意的是，`displaying` 方法的调用并不在主线程，所以你需要考虑多线程问题。**
...
调用 stop() 方法来停止展示弹幕并清理资源。与 DanmakuKit 相关的内存在该方法调用前不会释放。该方法会在 DanmakuView 销毁时调用。
...
如果想要在弹幕上展示 GIF，可以引入 GIF subspec，并使用 DanmakuGifCell 和 DanmakuGifCellModel。
...
DanmakuKit 通过 [CocoaPods](https://cocoapods.org) 安装。添加如下代码到你的 Podfile 文件中即可安装。
...
DanmakuKit 也支持通过 [Swift Package Manager](https://github.com/apple/swift-package-manager) 安装。添加如下依赖到你的 Package.Swift 文件即可安装：
...
DanmakuKit 使用 MIT 许可证，请查看 LICENSE 文件了解更多信息。
